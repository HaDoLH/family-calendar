...
- **歸屬關鍵字**（會蓋過切換鈕）：`女友 9/5 做指甲`、`共同 9/6 聽演唱會`

### 歸屬關鍵字

`共同`／`大家`／`全家`／`一起` → 共同行程；`個人`／`我` → 自己的；**成員名字** → 幫那個人排（代打）。

**位置自由、分隔符號可有可無**：`女友 9/5 做指甲`、`女友-9/5 做指甲`、`9/5 做指甲 女友` 三種都認得。

名字打一半也對得到（「女友」→「憨女友」），對到兩個以上就不猜。

⚠️ **關鍵字必須「獨立成詞」**（前後是空白、標點或句首句尾）。這是為了不要誤判標題：

| 輸入 | 結果 |
|---|---|
| `9/5 做指甲 女友` | 獨立成詞 → 歸給憨女友 |
| `9/25 女友家族吃火鍋` | 黏著「家族」→ 不算歸屬，標題完整保留 |
| `9/26 幫女友買禮物` | 同上 —— 那是你的事，不是她的事 |

`matchMember()` 只接受「完全相同」或「是某個成員名字的一部分」，反過來不行 —— 否則「幫女友買禮物」整串會被當成名字去比對。
